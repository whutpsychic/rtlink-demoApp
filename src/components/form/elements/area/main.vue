<template>
  <div class="rt-form-item">
    ...
    <van-popup v-model:show="showSelector" @close="checkVal" position="bottom" @open="openUp">
      <van-area v-model="pv" :area-list="areaListData" @confirm="onConfirm" @cancel="onCancel" />
    </van-popup>
  </div>
</template>
...
const expValue = computed(() => {
  ...
  return areaCalcer(areaList, modelValue);
});
...
const onConfirm = (v) => {
  ...
  emit("update:model-value", v.selectedValues[v.selectedValues.length - 1]);
  //
  checkVal();
};
...
const checkVal = () => {
  const { modelValue } = props;
  // 如果是必填且无值则报错警告
  if (isRequired() && (!modelValue || (modelValue && modelValue.length === 0))) {
    error.value = true;
  }
  // 否则静默
  else {
