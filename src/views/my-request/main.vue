<template>
  <rtm-app-container title="我的请求" canPop>
...
const tabc3 = ref(null);
const tabc4 = ref(null);
...
const onChangeTab = (index) => {
  ...
  if (tabc3.value && tabc3.value.resetSearchText) {
    tabc3.value.resetSearchText();
  }
  if (tabc4.value && tabc4.value.resetSearchText) {
    tabc4.value.resetSearchText();
  }
};
...
const onSearch = (text) => {
  if (active.value === 0) {
    ...
    tabc3.value.query(text);
  } else if (active.value === 3) {
    tabc4.value.query(text);
  }
};
